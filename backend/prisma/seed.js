...
const skuPrefixes = {
  ...
  Millets: "MIL",
  "Organic Staples": "ORG",
  "Cold Pressed Oils": "OIL",
  "Natural Spices": "SP",
  "Mitti Cookware": "MITTI"
};

const unitFromWeight = (weight) => {
  ...
  if (lower.includes("piece")) return "pieces";
  return "unit";
};

const skuFor = (name, category, weight) => {
  const prefix = skuPrefixes[category] || "PRD";
  const code = slugify(name, { lower: false, strict: true }).split("-").slice(0, 3).join("-");
  const size = weight.toUpperCase().replace(/\s+/g, "");
  return `${prefix}-${code}-${size}`;
};

const productSeo = ({ name, category, shortDescription, weight }) => ({
  metaTitle: `${name} ${weight} | Samruddhi`,
  metaDescription: `Buy ${name} ${weight} from Samruddhi. ${shortDescription}`,
  seoKeywords: [
    name.toLowerCase(),
    category.toLowerCase(),
    `buy ${name.toLowerCase()} online`,
    `samruddhi ${category.toLowerCase()}`
  ].join(", ")
});

const heroSlides = [
  ["Organic products", "Clean staples for daily Indian cooking", "Naturally sourced rice, jaggery, pulses, spices and pantry basics selected for freshness, taste and everyday usefulness.", "Organic Staples", "Shop organic staples", "/slide-organic-products.png", "Rice and pulses|Natural jaggery|Farm-style staples", 1],
  ["Dry fruits", "Premium cashews, almonds and festive mixes", "Creamy whole cashews, crunchy almonds, raisins and dry fruit blends for snacking, gifting and traditional sweets.", "Dry Fruits", "Shop dry fruits", "/slide-dry-fruits.png", "Whole cashews|Gift-ready mixes|Freshly packed", 2],
  ["Cold pressed oils", "Wood-pressed oils with natural character", "Small-batch groundnut, sesame and coconut oils made for tadka, chutneys, pickles and everyday home cooking.", "Cold Pressed Oils", "Shop cold pressed oils", "/slide-cold-pressed-oils.png", "Groundnut oil|Sesame oil|No refined blends", 3],
  ["Mitti products", "Terracotta cookware and cooling waterware", "Clay handis, tawa, curd pots, water bottles and matkas shaped for slow cooking, natural cooling and earthy table service.", "Mitti Cookware", "Shop mitti products", "/slide-mitti-products.png", "Clay handi|Water matka|Curd pots", 4]
];
...
async function main() {
  ...
  await prisma.user.upsert({
    ...
    update: {},
    create: { name: "Samruddhi Admin", email: "venukoyyana908@gmail.com", password: adminPassword, role: Role.ADMIN }
  });
  const categoryMap = {};
  ...
  for (const [name, category, price, discountPrice, weight, stock, featured, bestseller, shortDescription, image] of products) {
    ...
    const sku = skuFor(name, category, weight);
    const unit = unitFromWeight(weight);
    const seo = productSeo({ name, category, shortDescription, weight });
    await prisma.product.upsert({
      where: { slug },
      update: { sku, unit, ...seo },
      create: {
        name, slug, sku, unit, price, discountPrice, weight, stock, featured, bestseller,
  ...
  for (const [eyebrow, title, body, category, cta, image, highlights, sortOrder] of heroSlides) {
    const link = `/products?category=${categoryMap[category].slug}`;
    await prisma.$executeRaw`
      INSERT INTO "HeroSlide" (id, eyebrow, title, body, link, cta, image, highlights, "categoryId", "sortOrder", active, "updatedAt")
      VALUES (${sortOrder}, ${eyebrow}, ${title}, ${body}, ${link}, ${cta}, ${image}, ${highlights}, ${categoryMap[category].id}, ${sortOrder}, true, now())
  ...
  await prisma.$executeRaw`
    SELECT setval(
      pg_get_serial_sequence('"HeroSlide"', 'id'),
      COALESCE((SELECT MAX(id) FROM "HeroSlide"), 1),
      true
    )
  `;
}
