...
function ProductDetails() {
  ...
  return <main className="container-site py-12"><div className="mb-7 text-xs text-ink/45"><Link to="/">Home</Link> / <Link to="/products">Products</Link> / {product.name}</div>
    <div className="grid gap-10 lg:grid-cols-2">
      <div className="overflow-hidden rounded-[2rem] border border-forest/10 bg-white"><img src={product.image} alt={product.name} className="aspect-[4/3] w-full object-cover transition duration-500" onError={e=>e.currentTarget.src="/samruddhi-hero.png"}/></div>
      <div className="lg:py-5"><p className="eyebrow">{product.category.name}</p><h1 className="mt-2 text-4xl text-forest md:text-5xl">{product.name}</h1><div className="mt-4 flex items-center gap-2 text-xs"><span className="flex text-turmeric">{[1,2,3,4,5].map(i=><Star key={i} size={15} fill="currentColor"/>)}</span><span className="text-ink/45">4.9</span></div><p className="mt-6 text-3xl font-bold text-forest">{money(price)} {product.discountPrice&&<span className="ml-2 text-base font-normal text-ink/35 line-through">{money(product.price)}</span>}</p><p className="mt-1 text-md text-bold"> {product.weight}</p><p className="mt-7 leading-7 text-ink/65">{product.description}</p>
