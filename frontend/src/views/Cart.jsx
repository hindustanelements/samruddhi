...
function Cart() {
  ...
  return <main className="container-site py-12"><h1 className="text-4xl text-forest">Your basket</h1><p className="mt-2 text-sm text-ink/50">{cart.reduce((s,x)=>s+x.quantity,0)} items, packed with care</p><div className="mt-9 grid gap-8 lg:grid-cols-[1fr_380px]"><div className="space-y-4">{cart.map(item=><div key={item.id} className="card flex gap-4 p-4 sm:gap-6"><img src={item.image} className="aspect-[4/3] h-28 w-28 rounded-2xl border border-forest/10 bg-white object-cover" alt={item.name}/><div className="flex flex-1 flex-col justify-between py-1"><div className="flex justify-between gap-3"><div><p className="text-xs uppercase tracking-wider text-clay">{item.category?.name}</p><h3 className="mt-1 text-xl text-forest">{item.name}</h3><p className="text-xs text-ink/45">{item.weight}</p></div><button onClick={()=>remove(item.id)} className="text-ink/35 hover:text-red-600"><Trash2 size={18}/></button></div><div className="flex items-end justify-between"><div className="flex items-center gap-3 rounded-full border px-3 py-1.5"><button onClick={()=>update(item.id,item.quantity-1)}><Minus size={14}/></button><span className="text-sm font-bold">{item.quantity}</span><button onClick={()=>update(item.id,item.quantity+1)}><Plus size={14}/></button></div><strong>{money((item.discountPrice||item.price)*item.quantity)}</strong></div></div></div>)}</div>
