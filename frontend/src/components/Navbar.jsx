...
export default function Navbar() {
  ...
  const submit = (e) => { e.preventDefault(); navigate(getSearchPath(search)); setOpen(false); };
  useEffect(() => {
    ...
    <header className="sticky top-0 z-50 border-b" style={{ background: "var(--color-nav-bg)", borderColor: "color-mix(in srgb, var(--color-nav-text) 14%, transparent)", color: "var(--color-nav-text)" }}>
      <div className="container-site flex h-[70px] items-center justify-between gap-2 sm:gap-6 lg:gap-10">
        ...
        <div className="flex items-center gap-2">
          <form onSubmit={submit} className="relative hidden xl:block"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-white/55" size={15}/><input value={search} onChange={(e)=>{ setSearch(e.target.value); navigate(getSearchPath(e.target.value)); }} className="w-44 rounded-full py-1.5 pl-9 pr-4 text-xs text-white placeholder:text-white/55 ring-1 ring-white/15 transition focus:w-52" style={{ background: "var(--color-nav-surface)" }} placeholder="Search pantry..."/></form>
